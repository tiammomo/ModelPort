...
> **状态：Proposed / Not implemented.** 本文是未来独立图像能力的设计提案，
> 不是当前 ModelPort API、Provider 支持或发布承诺。当前权威范围见
> [API](API.md) 与 [Architecture](ARCHITECTURE.md)。外部模型名称、能力和价格
> 会变化；实现前必须重新核对官方资料。最后审阅：2026-07-11。

这份文档用于指导 ModelPort 后续接入 `gpt-image-2`。当前 ModelPort 的数据面是
Anthropic-compatible `/v1/messages` 与有明确范围的 OpenAI-compatible
`/v1/chat/completions` 文本协议；`gpt-image-2` 应作为独立图像能力扩展，不应
混入现有文本生成链路。
...
`README.md` 和 `README.zh-CN.md` 当前共用架构 SVG。快速上手 SVG 保留为可选资产，
但目前未在 README 引用；不能把未引用资产描述成当前入口的一部分。SVG 比外部生成
图片更易长期渲染和维护，但里面的模型名、命令和状态仍必须随代码复核。
...
```text
Create a clean technical README architecture infographic for an open source developer tool called "ModelPort". Wide 16:9 aspect ratio, modern GitHub README style, crisp readable labels, generous spacing. Visual concept: Claude Code / VS Code Claude and OpenAI-compatible SDKs on the left send Anthropic Messages or Chat Completions to a local gateway box labeled "ModelPort" in the center, then ModelPort routes to configured providers on the right: DeepSeek v4 flash, Anthropic, OpenAI-compatible, OpenRouter, Ollama / Custom. Show only implemented ideas as small badges: Client API key, Typed Exchange IR, Model routing, Protocol transform, Terminal streaming SSE, Health and metrics. Use a professional light theme with white background, deep teal and graphite accents. Avoid clutter, no fake UI screenshots, no tiny unreadable text, no unsupported fallback claim, no external logos.
```
